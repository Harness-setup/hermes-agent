...
class CLIChatTurnMixin:
    ...
    def _chat_print_response_panel(self, turn, response):
        ...
        if response and not (turn.result and turn.result.get("response_previewed", False)):
            ...
            # already streamed token-by-token to the terminal -- streaming necessarily renders
            # before finalize_turn() runs that hook. Verified live (original repro: session
            # 20260723_143132_0dd814): the swap was correctly persisted to the durable transcript,
            # but the interactive CLI kept showing the pre-swap text forever, since this box was
            # skipped unconditionally whenever anything had streamed. Force the full Panel render
            # in that case so the user actually sees the corrected answer, not just the session
            # history. (Re-derived multiple times after `hermes update` resets wiped it; see
            # agent/turn_finalizer.py's transform_llm_output hook comment and the switch to a
            # dedicated `local-fixes` branch.)
