...
def _apply_transform_hook(agent, final_response, logger, *, platform) -> Tuple[Any, bool, Optional[Any]]:
    """Fire ``transform_llm_output`` (first hook to return a string wins; None/empty leaves the
    text unchanged). Returns ``(final_response, transformed, pre_transform_response)``.

    Must run BEFORE persistence (called from ``_persist_step`` in ``finalize_turn``, right after
    ``final_response`` is stream-recovered and before ``_close_transcript_tail``/
    ``agent._persist_session``): ``_session_db.append_message`` is a pure insert with no update
    path, so a swap applied after the one-and-only persist call for this turn can't be written
    back into the same row without creating a duplicate. Running it here means the durable
    transcript, the trajectory log, and the returned result all agree on the (possibly swapped)
    text. Caller gates this on a real, uninterrupted response -- transforming nothing doesn't
    make sense."""
    for _hook_result in _invoke_hook_safely(
        ...
        if isinstance(_hook_result, str) and _hook_result:
            return _hook_result, True, final_response
...
def finalize_turn(
    ...
    def _persist_step():
        ...
        if final_response and not interrupted:
            final_response, _response_transformed, _pre_transform_response = _apply_transform_hook(
