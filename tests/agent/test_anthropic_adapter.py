...
class TestApiKeyConstructionClearsEnvBearerToken:
    ...
    def test_bearer_style_client_keeps_its_auth_token(self, monkeypatch):
        ...
        # Same Omit()-header guard as the sibling test above, mirrored: the SDK independently
        # fills BOTH api_key (from ANTHROPIC_API_KEY) and client.auth_headers with it -- neither
        # is itself a leak, since default_headers/Omit() only applies at request-build time.
        # What must never happen is x-api-key actually reaching the wire alongside Authorization.
        for wire_client in (client, client.with_options(timeout=30)):
            headers = dict(wire_client._build_headers(_final_request_options(anthropic_sdk)))
